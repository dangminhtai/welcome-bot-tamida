...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub =>
            sub.setName("list").setDescription("Show all configured roles"))
        .addSubcommand(sub =>
            sub.setName("send-menu").setDescription("Send the role selection menu")),
    ...
    async execute(interaction) {
        ...
        if (sub === "add") {
            ...
            if (exists) return interaction.reply({ content: "❌ Role already exists in config!", ephemeral: true });
        ...
        if (sub === "list") {
            ...
            if (!configs.length) return interaction.reply({ content: "No roles configured yet.", ephemeral: true });
