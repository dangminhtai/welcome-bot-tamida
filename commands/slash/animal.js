...
const { SlashCommandBuilder, AttachmentBuilder, EmbedBuilder } = discord;
...
async function getRedditPost(topic) {
    ...
    try {
        ...
        if (validPosts.length === 0)
            throw new Error('Không tìm thấy ảnh hợp lệ');
...
export const data = new SlashCommandBuilder()
    ...
    .addStringOption(option => option.setName('type')
        ...
        .setRequired(false)
        .addChoices({ name: 'Tất cả', value: 'all' }, { name: 'Chó', value: 'dog' }, { name: 'Mèo', value: 'cat' }, { name: 'Thỏ', value: 'rabbit' }, { name: 'Cáo', value: 'fox' }, { name: 'Chim', value: 'bird' }, { name: 'Sóc', value: 'squirrel' }));
