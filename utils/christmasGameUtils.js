...
const { ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, EmbedBuilder } = discord;
...
const createGameUI = (gameData, hostName, hostAvatar) => {
    ...
    let description = TREE_CONFIG.descriptions.active(Array.from(gameData.scores.entries())
        .map(([id, s]) => `${gameData.names.get(id)}: ${s}`)
        .join(', ') || "Chưa có", gameData.treeState);
    ...
    if (!gameData.stopRequesterId) {
        ...
        const rowButtons = new ActionRowBuilder()
            .addComponents(new ButtonBuilder().setCustomId(TREE_CONFIG.buttons.customIds.candy).setEmoji(TREE_CONFIG.items.candy.emoji).setLabel('Kẹo').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(TREE_CONFIG.buttons.customIds.bow).setEmoji(TREE_CONFIG.items.bow.emoji).setLabel('Nơ').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(TREE_CONFIG.buttons.customIds.gift).setEmoji(TREE_CONFIG.items.gift.emoji).setLabel('Quà').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId(TREE_CONFIG.buttons.customIds.heart).setEmoji(TREE_CONFIG.items.heart.emoji).setLabel('Tim').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId(TREE_CONFIG.buttons.customIds.stop).setEmoji('🛑').setLabel('Kết thúc').setStyle(ButtonStyle.Danger));
